...
# The CMD server's TLS chain is not publicly verifiable; pin the host and
# skip verification for this one downloader.
import urllib3
...
class ParsecQuery:
    ...
    def query(self, ret_table=True, **args):
        ...
        **args : dict
        ...
        """
        r = requests.post(self.website, data={**self.args_default, **args}, verify=_VERIFY)
        p = etree.HTML(r.content)
        url_output = p.xpath("//a[contains(text(), 'output')]/@href")
        if url_output:
            url = urljoin(self.website, url_output[0])
            output = requests.get(url, verify=_VERIFY).content
            if url.endswith(".gz"):
    ...
    def _get_args(self):
        ...
        """
        r = requests.get(self.website, verify=_VERIFY)
        p = etree.HTML(r.content)
